...
export const subscriptions = pgTable("subscriptions", {
  ...
  monthly_ai_budget: decimal("monthly_ai_budget", { precision: 10, scale: 4 })
    .notNull()
    .default("0"), // Monthly AI budget in EUR (custom_price * 0.25)
...
export const uploadedFilesTable = pgTable("uploaded_files", {
  ...
  user_id: text("user_id")
    .notNull()
    .references(() => users.id),
  file_name: text("file_name").notNull(), // Original filename for display
  pinecone_source: text("pinecone_source").notNull(), // Unique filename used in Pinecone (for filtering)
  file_type: text("file_type").notNull(), // pdf, docx, txt
  title: text("title").notNull(), // Generated by LLM
  description: text("description").notNull(), // Generated by LLM
  n_pages: integer("n_pages").notNull().default(1), // For PDFs, or estimated pages for other formats
  storage_path: text("storage_path"), // Optional: if we store files in cloud storage
...
export const aiUsageTable = pgTable("ai_usage", {
  ...
  input_cost_usd: decimal("input_cost_usd", { precision: 10, scale: 8 })
    .notNull()
    .default("0"), // Cost in USD for input tokens
  output_cost_usd: decimal("output_cost_usd", { precision: 10, scale: 8 })
    .notNull()
    .default("0"), // Cost in USD for output tokens
  total_cost_usd: decimal("total_cost_usd", { precision: 10, scale: 8 })
    .notNull()
    .default("0"), // Total cost in USD
  chat_type: text("chat_type").notNull(), // 'pdf', 'subject', 'dashboard'
  model_used: text("model_used").notNull().default("deepseek-chat"),
  created_at: timestamp("created_at").notNull().defaultNow(),
});

export const aiBudgetBalanceTable = pgTable(
  "ai_budget_balance",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    user_id: text("user_id")
    ...
    allocated_budget_eur: decimal("allocated_budget_eur", {
      precision: 10,
      scale: 4,
    }).notNull(), // Monthly budget in EUR
    used_budget_usd: decimal("used_budget_usd", { precision: 10, scale: 8 })
      .notNull()
      .default("0"), // Used budget in USD
    remaining_budget_eur: decimal("remaining_budget_eur", {
      precision: 10,
      scale: 4,
    }).notNull(), // Remaining budget in EUR
    created_at: timestamp("created_at").notNull().defaultNow(),
    updated_at: timestamp("updated_at").notNull().defaultNow(),
  },
  (t) => ({
    uniqPeriod: unique("uniq_ai_budget_period").on(
      t.user_id,
      t.subscription_id,
      t.period_start,
      t.period_end
